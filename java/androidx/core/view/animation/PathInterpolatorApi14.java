...
class PathInterpolatorApi14 implements Interpolator {
    ...
    /* renamed from: mX */
    private final float[] f4642mX;

    /* renamed from: mY */
    private final float[] f4643mY;
    ...
    public PathInterpolatorApi14(Path path) {
        ...
        int i2 = i + 1;
        this.f4642mX = new float[i2];
        this.f4643mY = new float[i2];
        float[] fArr = new float[2];
        for (int i3 = 0; i3 < i2; i3++) {
            pathMeasure.getPosTan((i3 * length) / i, fArr, null);
            this.f4642mX[i3] = fArr[0];
            this.f4643mY[i3] = fArr[1];
        }
    }
    ...
    public float getInterpolation(float f) {
        ...
        int length = this.f4642mX.length - 1;
        int i = 0;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f4642mX[i2]) {
                length = i2;
            } else {
                i = i2;
            }
        }
        float[] fArr = this.f4642mX;
        float f2 = fArr[length];
        float f3 = fArr[i];
        float f4 = f2 - f3;
        if (f4 == 0.0f) {
            return this.f4643mY[i];
        }
        float f5 = (f - f3) / f4;
        float[] fArr2 = this.f4643mY;
        float f6 = fArr2[i];
