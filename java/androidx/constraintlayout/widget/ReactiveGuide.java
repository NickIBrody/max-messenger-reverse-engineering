...
public class ReactiveGuide extends View implements C0774c.a {
    ...
    private void changeValue(int i, int i2, MotionLayout motionLayout, int i3) {
        ...
        constraintSet.m4562Z(i2, i);
        motionLayout.updateState(i3, constraintSet);
    }

    private void init(AttributeSet attributeSet) {
        if (attributeSet != null) {
            TypedArray obtainStyledAttributes = getContext().obtainStyledAttributes(attributeSet, tuf.ConstraintLayout_ReactiveGuide);
            int indexCount = obtainStyledAttributes.getIndexCount();
            for (int i = 0; i < indexCount; i++) {
                int index = obtainStyledAttributes.getIndex(i);
                if (index == tuf.ConstraintLayout_ReactiveGuide_reactiveGuide_valueId) {
                    this.mAttributeId = obtainStyledAttributes.getResourceId(index, this.mAttributeId);
                } else if (index == tuf.ConstraintLayout_ReactiveGuide_reactiveGuide_animateChange) {
                    this.mAnimateChange = obtainStyledAttributes.getBoolean(index, this.mAnimateChange);
                } else if (index == tuf.f106582x2694048c) {
                    this.mApplyToConstraintSetId = obtainStyledAttributes.getResourceId(index, this.mApplyToConstraintSetId);
                } else if (index == tuf.f106581xfdeff96) {
        ...
        if (this.mAttributeId != -1) {
            ConstraintLayout.getSharedValues().m4611a(this.mAttributeId, this);
        }
    }
    ...
    public void onNewValue(int i, int i2, int i3) {
        ...
        if (id > 0 && (getParent() instanceof MotionLayout)) {
            ...
            C0773b cloneConstraintSet = motionLayout.cloneConstraintSet(currentState);
            cloneConstraintSet.m4562Z(id, i2);
    ...
    public void setAttributeId(int i) {
        C0774c sharedValues = ConstraintLayout.getSharedValues();
        int i2 = this.mAttributeId;
        if (i2 != -1) {
            sharedValues.m4612b(i2, this);
        }
        this.mAttributeId = i;
        if (i != -1) {
            sharedValues.m4611a(i, this);
        }
    }
