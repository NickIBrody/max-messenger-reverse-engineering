...
public class CircularFlow extends VirtualLayout {
    ...
    public void init(AttributeSet attributeSet) {
        ...
        if (attributeSet != null) {
            ...
            for (int i = 0; i < indexCount; i++) {
                ...
                if (index == tuf.ConstraintLayout_Layout_circularflow_viewCenter) {
                    this.mViewCenter = obtainStyledAttributes.getResourceId(index, 0);
                } else if (index == tuf.ConstraintLayout_Layout_circularflow_angles) {
                    String string = obtainStyledAttributes.getString(index);
                    this.mReferenceAngles = string;
                    setAngles(string);
                } else if (index == tuf.ConstraintLayout_Layout_circularflow_radiusInDP) {
                    String string2 = obtainStyledAttributes.getString(index);
                    this.mReferenceRadius = string2;
                    setRadius(string2);
                } else if (index == tuf.ConstraintLayout_Layout_circularflow_defaultAngle) {
                    Float valueOf = Float.valueOf(obtainStyledAttributes.getFloat(index, DEFAULT_ANGLE));
                    this.mReferenceDefaultAngle = valueOf;
                    setDefaultAngle(valueOf.floatValue());
                } else if (index == tuf.ConstraintLayout_Layout_circularflow_defaultRadius) {
    ...
    public int removeView(View view) {
        ...
        c0773b.m4578p(this.mContainer);
        c0773b.m4576n(view.getId(), 8);
        c0773b.m4572i(this.mContainer);
        float[] fArr = this.mAngles;
