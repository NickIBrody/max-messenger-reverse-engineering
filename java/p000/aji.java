package p000;

import android.text.TextUtils;

/* loaded from: classes3.dex */
public final class aji {

    /* renamed from: a */
    public final int f2175a;

    /* renamed from: b */
    public final int f2176b;

    /* renamed from: c */
    public final int f2177c;

    /* renamed from: d */
    public final int f2178d;

    /* renamed from: e */
    public final int f2179e;

    public aji(int i, int i2, int i3, int i4, int i5) {
        this.f2175a = i;
        this.f2176b = i2;
        this.f2177c = i3;
        this.f2178d = i4;
        this.f2179e = i5;
    }

    /* JADX WARN: Can't fix incorrect switch cases order, some code will duplicate */
    /* renamed from: a */
    public static aji m1854a(String str) {
        char c;
        l00.m48470a(str.startsWith("Format:"));
        String[] split = TextUtils.split(str.substring(7), ",");
        int i = -1;
        int i2 = -1;
        int i3 = -1;
        int i4 = -1;
        for (int i5 = 0; i5 < split.length; i5++) {
            String m114615f = AbstractC17723yy.m114615f(split[i5].trim());
            m114615f.getClass();
            switch (m114615f.hashCode()) {
                case 100571:
                    if (m114615f.equals("end")) {
                        c = 0;
                        break;
                    }
                    c = 65535;
                    break;
                case 3556653:
                    if (m114615f.equals("text")) {
                        c = 1;
                        break;
                    }
                    c = 65535;
                    break;
                case 109757538:
                    if (m114615f.equals("start")) {
                        c = 2;
                        break;
                    }
                    c = 65535;
                    break;
                case 109780401:
                    if (m114615f.equals("style")) {
                        c = 3;
                        break;
                    }
                    c = 65535;
                    break;
                default:
                    c = 65535;
                    break;
            }
            switch (c) {
                case 0:
                    i2 = i5;
                    break;
                case 1:
                    i4 = i5;
                    break;
                case 2:
                    i = i5;
                    break;
                case 3:
                    i3 = i5;
                    break;
            }
        }
        if (i == -1 || i2 == -1 || i4 == -1) {
            return null;
        }
        return new aji(i, i2, i3, i4, split.length);
    }
}
