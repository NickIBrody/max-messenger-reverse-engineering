...
public class BasicPathHandler implements CookieAttributeHandler {
    ...
    public boolean match(Cookie cookie, CookieOrigin cookieOrigin) {
        ...
        if (path2 == null) {
            path2 = CSPStore.SLASH;
        }
        if (path2.length() > 1 && path2.endsWith(CSPStore.SLASH)) {
            path2 = path2.substring(0, path2.length() - 1);
        }
        boolean startsWith = path.startsWith(path2);
        return (!startsWith || path.length() == path2.length() || path2.endsWith(CSPStore.SLASH)) ? startsWith : path.charAt(path2.length()) == '/';
    }
