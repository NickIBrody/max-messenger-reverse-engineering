...
abstract class CameraCapturer implements CameraVideoCapturer {
    ...
    public void checkIsOnCameraThread() {
        ...
        Logging.m81556e(TAG, "Check is on camera thread failed.");
    ...
    public void switchCameraInternal(CameraVideoCapturer.CameraSwitchHandler cameraSwitchHandler, String str) {
        Logging.m81555d(TAG, "switchCamera internal");
        ...
        synchronized (this.stateLock) {
            try {
                ...
                Logging.m81555d(TAG, "switchCamera: Stopping session");
                ...
                Logging.m81555d(TAG, "switchCamera done");
            } catch (Throwable th) {
                throw th;
            }
        }
    ...
    public void changeCaptureFormat(int i, int i2, int i3) {
        Logging.m81555d(TAG, "changeCaptureFormat: " + i + "x" + i2 + "@" + i3);
        synchronized (this.stateLock) {
            stopCapture();
    ...
    public void printStackTrace() {
        ...
        if (thread != null) {
            ...
            if (stackTrace.length > 0) {
                Logging.m81555d(TAG, "CameraCapturer stack trace:");
                for (StackTraceElement stackTraceElement : stackTrace) {
                    Logging.m81555d(TAG, stackTraceElement.toString());
                }
            }
        }
    }
    ...
    public void startCapture(int i, int i2, int i3) {
        Logging.m81555d(TAG, "startCapture: " + i + "x" + i2 + "@" + i3);
        ...
        synchronized (this.stateLock) {
            try {
                ...
                Logging.m81559w(TAG, "Session already open");
            } catch (Throwable th) {
                throw th;
            }
        }
    ...
    public void stopCapture() {
        Logging.m81555d(TAG, "Stop capture");
        synchronized (this.stateLock) {
            while (this.sessionOpening) {
                Logging.m81555d(TAG, "Stop capture: Waiting for session to open");
                try {
                    this.stateLock.wait();
                } catch (InterruptedException unused) {
                    Logging.m81559w(TAG, "Stop capture interrupted while waiting for the session to open.");
                    Thread.currentThread().interrupt();
                    return;
                }
            }
            if (this.currentSession != null) {
                Logging.m81555d(TAG, "Stop capture: Nulling session");
                this.cameraStatistics.release();
                this.cameraStatistics = null;
                final CameraSession cameraSession = this.currentSession;
                ...
                Logging.m81555d(TAG, "Stop capture: No session open");
            }
        }
        Logging.m81555d(TAG, "Stop capture done");
    }

    @Override // org.webrtc.CameraVideoCapturer
    public void switchCamera(final CameraVideoCapturer.CameraSwitchHandler cameraSwitchHandler) {
        Logging.m81555d(TAG, "switchCamera");
    ...
    public CameraCapturer(String str, CameraVideoCapturer.CameraEventsHandler cameraEventsHandler, CameraEnumerator cameraEnumerator, CameraVideoCapturer.CameraConfigurationProvider cameraConfigurationProvider, CameraVideoCapturer.CaptureFormatHelper captureFormatHelper) {
        this.createSessionCallback = new CameraSession.CreateSessionCallback() { // from class: org.webrtc.CameraCapturer.1
            ...
            public void onDone(CameraSession cameraSession) {
                ...
                Logging.m81555d(CameraCapturer.TAG, "Create session done. Switch state: " + String.valueOf(CameraCapturer.this.switchState));
                ...
                synchronized (CameraCapturer.this.stateLock) {
                    try {
                        ...
                        if (CameraCapturer.this.openAttemptsRemaining <= 0) {
                            Logging.m81559w(CameraCapturer.TAG, "Opening camera failed, passing: " + str2);
                            CameraCapturer.this.sessionOpening = false;
                            CameraCapturer.this.stateLock.notifyAll();
                            ...
                            Logging.m81559w(CameraCapturer.TAG, "Opening camera failed, retry: " + str2);
        ...
        this.cameraSessionEventsHandler = new CameraSession.Events() { // from class: org.webrtc.CameraCapturer.2
                ...
                synchronized (CameraCapturer.this.stateLock) {
                    try {
                        if (cameraSession == CameraCapturer.this.currentSession || CameraCapturer.this.currentSession == null) {
                            ...
                            Logging.m81555d(CameraCapturer.TAG, "onCameraClosed from another session.");
                        }
                    } catch (Throwable th) {
                        throw th;
                    }
                ...
                synchronized (CameraCapturer.this.stateLock) {
                    try {
                        if (cameraSession != CameraCapturer.this.currentSession) {
                            Logging.m81559w(CameraCapturer.TAG, "onCameraDisconnected from another session.");
                        } else {
                            CameraCapturer.this.eventsHandler.onCameraDisconnected();
                ...
                synchronized (CameraCapturer.this.stateLock) {
                    try {
                        if (cameraSession == CameraCapturer.this.currentSession) {
                            ...
                            Logging.m81559w(CameraCapturer.TAG, "onCameraError from another session: " + str2);
                        }
                    } catch (Throwable th) {
                        throw th;
                    }
                ...
                synchronized (CameraCapturer.this.stateLock) {
                    try {
                        if (CameraCapturer.this.currentSession != null) {
                            Logging.m81559w(CameraCapturer.TAG, "onCameraOpening while session was open.");
                        } else {
            ...
            public void onFrameCaptured(CameraSession cameraSession, VideoFrame videoFrame) {
                ...
                synchronized (CameraCapturer.this.stateLock) {
                    try {
                        if (cameraSession != CameraCapturer.this.currentSession) {
                            Logging.m81559w(CameraCapturer.TAG, "onFrameCaptured from another session.");
                            return;
                        }
            ...
            public void onNonFatal(CameraSession cameraSession, String str2, Throwable th) {
                ...
                synchronized (CameraCapturer.this.stateLock) {
                    try {
                        if (cameraSession != CameraCapturer.this.currentSession) {
                            Logging.m81559w(CameraCapturer.TAG, "onNonFatal from another session: " + str2);
                        }
