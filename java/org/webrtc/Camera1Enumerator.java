...
public class Camera1Enumerator implements CameraEnumerator {
    ...
    private static List<CameraEnumerationAndroid.CaptureFormat> enumerateFormats(int i) {
        int i2;
        Logging.m81555d(TAG, "Get supported formats for camera index " + i + Extension.DOT_CHAR);
        long elapsedRealtime = SystemClock.elapsedRealtime();
        Camera camera = null;
        try {
            try {
                Logging.m81555d(TAG, "Opening camera with index " + i);
                ...
                try {
                    ...
                    Logging.m81557e(TAG, "getSupportedFormats() failed on camera index " + i, e);
                }
                Logging.m81555d(TAG, "Get supported formats for camera index " + i + " done. Time spent: " + (SystemClock.elapsedRealtime() - elapsedRealtime) + " ms.");
                return arrayList;
            } catch (RuntimeException e2) {
                Logging.m81557e(TAG, "Open camera failed on camera index " + i, e2);
    ...
    public static int getCameraIndex(String str) {
        Logging.m81555d(TAG, "getCameraIndex: " + str);
        for (int i = 0; i < Camera.getNumberOfCameras(); i++) {
            if (str.equals(getDeviceName(i))) {
                return i;
            }
        }
        throw new IllegalArgumentException("No such camera: " + str + ", have: " + Arrays.toString(getDeviceNamesS()));
    }
    ...
    private static Camera.CameraInfo getCameraInfo(int i) {
        ...
        try {
            ...
            Logging.m81557e(TAG, "getCameraInfo failed on index " + i, e);
            return null;
        }
    }
    ...
    private static String[] getDeviceNamesS() {
        ...
        for (int i = 0; i < Camera.getNumberOfCameras(); i++) {
            ...
            if (deviceName != null) {
                ...
                Logging.m81555d(TAG, "Index: " + i + ". " + deviceName);
            } else {
                Logging.m81556e(TAG, "Index: " + i + ". Failed to query camera name.");
            }
        }
