    ...
    public void maybeAdjustBufferSize(AudioTrack audioTrack) {
        ...
        if (underrunCount > this.prevUnderrunCount) {
            if (this.bufferIncreaseCounter < 5) {
                ...
                Logging.m81555d(TAG, "Underrun detected! Increasing AudioTrack buffer size from " + bufferSizeInFrames + " to " + playbackRate);
        ...
        if (this.keepLoweringBufferSize) {
            ...
            if (i <= 0) {
                ...
                if (max != bufferSizeInFrames2) {
                    Logging.m81555d(TAG, "Lowering AudioTrack buffer size from " + bufferSizeInFrames2 + " to " + max);
