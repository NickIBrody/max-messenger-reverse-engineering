...
class WebRtcSilenceProvider {
    ...
    public static class Statistics {
        ...
        public void trackRead(long j, long j2, long j3) {
            ...
            long j4 = this.totalReadTimeNs + (j2 - j);
            ...
            if (j5 > j6) {
                ...
                Logging.m81555d(WebRtcSilenceProvider.TAG, "Log interval: " + j6 + "ns, log delta: " + j5 + "ns, reads: " + this.readCount + ", read time: " + millis + "ms, suspend time: " + j7 + "ms");
                reset();
            }
        }
    ...
    public WebRtcSilenceProvider(int i, int i2, int i3, int i4, int i5, byte[] bArr) {
        ...
        Logging.m81555d(TAG, "Silence provider initialized, sampleRate=" + i2 + ", framesPerBuffer=" + i4 + ", bufferDuration=" + TimeUnit.NANOSECONDS.toMillis(nanos) + "ms, bufferCapacity=" + bArr.length);
    }
    ...
    public int read(ByteBuffer byteBuffer, int i) {
        ...
        if (millis > 0) {
            try {
                ...
                Logging.m81555d(TAG, "Interrupted while waiting for frame duration, return immediately");
            }
        }
