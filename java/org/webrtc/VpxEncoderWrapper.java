...
public class VpxEncoderWrapper implements EncoderCallback {
    ...
    public void onEncodedImage(EncodedImage encodedImage) {
        if (this.consumerCallback != null) {
            ...
            this.consumerCallback.onEncodedImage(EncodedImage.builder().setBuffer(allocate, null).setEncodedHeight(encodedImage.encodedHeight).setEncodedWidth(encodedImage.encodedWidth).setFrameType(encodedImage.frameType).setQp(encodedImage.f82936qp).setRotation(encodedImage.rotation).createEncodedImage());
