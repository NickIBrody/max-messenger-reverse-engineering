...
public class ScreenCapturerAndroid implements VideoCapturer, VideoSink {
    ...
    private final Runnable rerequestTextureFrame = new Runnable() { // from class: h0h
        @Override // java.lang.Runnable
        public final void run() {
            ScreenCapturerAndroid.this.lambda$new$0();
        }
    };
    ...
    private VideoFrame adjustTs(VideoFrame videoFrame) {
        ...
        if (timestampNs > j2) {
            j = j2 == 0 ? videoFrame.getTimestampNs() : Math.max(videoFrame.getTimestampNs(), this.lastNewFrameTimestamp + (elapsedRealtimeNanos - this.lastNewFrameSystemtime));
            this.lastNewFrameTimestamp = j;
            this.lastNewFrameSystemtime = elapsedRealtimeNanos;
        } else {
            j = j2 + (elapsedRealtimeNanos - this.lastNewFrameSystemtime);
        }
    ...
    public synchronized void changeCaptureFormat(int i, int i2, int i3) {
        ...
        ThreadUtils.invokeAtFrontUninterruptibly(this.surfaceTextureHelper.getHandler(), new Runnable() { // from class: i0h
            @Override // java.lang.Runnable
            public final void run() {
